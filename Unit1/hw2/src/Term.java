...
public class Term {
    ...
    public BigInteger GetTermGcd(BigInteger oldGcd) {
        Unit unit = (Unit) this.factorList.get(0);
        return unit.GetUnitGcd(oldGcd);
    }

    public Term GetTermDivideNum(BigInteger num) {
        Term term = new Term();
        Unit unit = (Unit) this.factorList.get(0);
        term.factorList.add(unit.GetUnitDivideNum(num));
        return term;
    }
    ...
    public boolean equals(Term otherTerm) {
        if (this.factorList.size() > 1 || otherTerm.factorList.size() > 1) {
            ...
            return thisUnit.equals(otherUnit);
        }
    }
