...
public interface Tag {
    ...
    //@ ensures \result ==
    //      (\exists int i; 0 <= i && i < persons.length; persons[i].equals(person));
    public /*@ pure @*/ boolean hasPerson(Person person);

    /*@ ensures \result ==
                (\sum int i; 0 <= i && i < persons.length;
          @         (\sum int j; 0 <= j && j < persons.length && persons[i].isLinked(persons[j]);
                    persons[i].queryValue(persons[j])));
      @*/
    ...
    /*@ ensures \result == (persons.length == 0?
                0 : ((\sum int i; 0 <= i && i < persons.length;
