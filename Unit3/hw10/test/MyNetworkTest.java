...
public class MyNetworkTest {
    ...
    public void queryCoupleSum()
        throws EqualPersonIdException, PersonIdNotFoundException,
        EqualRelationException, RelationNotFoundException {
        MyNetwork network = new MyNetwork();
        for (int i = 0; i < 100; i++) {
        ...
        Person[] oldList = network.getPersons();
        assertEquals(network.queryCoupleSum(), 0);
        Person[] newList = network.getPersons();
        assertEquals(oldList.length, newList.length);
        for (int i = 0; i < oldList.length; i++) {
            MyPerson person = (MyPerson) oldList[i];
        ...
        network.addRelation(2, 4, 2);
        network.addRelation(1, 2, 1);
        assertEquals(network.queryCoupleSum(), 2);

        network.modifyRelation(1, 2, 3);
        assertEquals(network.queryCoupleSum(), 1);
        ...
        network.addRelation(5,6,1);
        assertEquals(network.queryCoupleSum(), 3);
        network.modifyRelation(5,6,2);
        assertEquals(network.queryCoupleSum(),3);
        network.modifyRelation(5,6,-1);
        assertEquals(network.queryCoupleSum(),3);
        network.modifyRelation(5,6,-3);
        assertEquals(network.queryCoupleSum(),2);

        network.addRelation(5,6,2);
        network.addRelation(5,7,1);
        assertEquals(network.queryCoupleSum(),3);

        network.modifyRelation(5,7,3);
        assertEquals(network.queryCoupleSum(),3);

        network.addRelation(13,14,100);
        network.addRelation(9,10,10);
        network.addRelation(10,13,2);
        assertEquals(network.queryCoupleSum(),5);

        network.modifyRelation(9,10,-9);
        assertEquals(network.queryCoupleSum(),4);
    }
}
