...
public interface Network {
    ...
    /*@ invariant persons != null &&
        (\forall int i,j; 0 <= i && i < j && j < persons.length; !persons[i].equals(persons[j])) &&
                  messages != null &&
                  (\forall int i,j; 0 <= i && i < j && j < messages.length; !messages[i].equals(messages[j])) &&
    ...
    /*@ public normal_behavior
      ...
      @ signals (PersonIdNotFoundException e) containsPerson(personId1) &&
                                            ...
                                            !getPerson(personId2).getTag(tagId).hasPerson(getPerson(personId1));
      @*/
    public /*@ safe @*/ void delPersonFromTag(int personId1, int personId2,int tagId)
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\forall int i;
                         0 <= i && i < \old(getMessage(id).getPerson2().getMessages().size());
      ...
      @ ensures (\forall Person p;
                        \old(getMessage(id)).getTag().hasPerson(p);
                        p.getSocialValue() == \old(p.getSocialValue()) + \old(getMessage(id)).getSocialValue());
      @ ensures (\old(getMessage(id)) instanceof RedEnvelopeMessage)
                    && (\old(getMessage(id)).getTag().getSize() > 0)
                ==>
                 (\exists int i;
                          i == ((RedEnvelopeMessage)\old(getMessage(id))).getMoney() /
                                    \old(getMessage(id)).getTag().getSize();
                          \old(getMessage(id)).getPerson1().getMoney() ==
                                \old(getMessage(id).getPerson1().getMoney())
                                    - i * \old(getMessage(id)).getTag().getSize()
                &&
                  (\forall Person p;
                           \old(getMessage(id)).getTag().hasPerson(p);
      ...
      @ ensures (\old(getMessage(id)) instanceof EmojiMessage) ==>
                    (\exists int i;
                             0 <= i && i < emojiIdList.length
                                && emojiIdList[i] == ((EmojiMessage)\old(getMessage(id))).getEmojiId();
                            emojiHeatList[i] == \old(emojiHeatList[i]) + 1);
      ...
      @ signals (RelationNotFoundException e) containsMessage(id) &&
                    getMessage(id).getType() == 0 &&
                    !(getMessage(id).getPerson1().isLinked(getMessage(id).getPerson2()));
      @ signals (TagIdNotFoundException e) containsMessage(id) &&
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\exists int i;
                         0 <= i && i < emojiIdList.length;
                         emojiIdList[i] == id && emojiHeatList[i] == 0);
      @ ensures emojiIdList.length == \old(emojiIdList.length) + 1
                    && emojiHeatList.length == \old(emojiHeatList.length) + 1;
      @ ensures (\forall int i;
                         0 <= i && i < \old(emojiIdList.length);
                         (\exists int j;
                                  0 <= j && j < emojiIdList.length;
                                  emojiIdList[j] == \old(emojiIdList[i])
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\forall int i;
                         0 <= i && i < emojiIdList.length;
                         (\exists int j;
                                  0 <= j && j < \old(emojiIdList.length);
                                  emojiIdList[i] == \old(emojiIdList[j])
                                        && emojiHeatList[i] == \old(emojiHeatList[j])));
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\forall Message i;
                         \old(getPerson(personId).getMessages().contains(i));
                         !(i instanceof NoticeMessage)
                            ==> (\exists Message j;
                                         getPerson(personId).getMessages().contains(j);
                                         i.equals(j)));
      @ ensures getPerson(personId).messages.length ==
                  (\num_of int i;
                           0 <= i && i < \old(getPerson(personId).messages.length);
                           !(\old(getPerson(personId).getMessages().get(i)) instanceof NoticeMessage));
      @ ensures (\forall int i;
                         0 <= i && i < getPerson(personId).getMessages().size();
                         (\forall int j;
                                  i < j && j < getPerson(personId).getMessages().size();
    ...
    /*@ ensures \result ==
                (\sum int i, j; 0 <= i && i < j && j < persons.length
                                && persons[i].acquaintance.length > 0 &&
                                    queryBestAcquaintance(persons[i].getId()) == persons[j].getId()
                                && persons[j].acquaintance.length > 0 &&
                                    queryBestAcquaintance(persons[j].getId()) == persons[i].getId();
                                1);
      @*/
