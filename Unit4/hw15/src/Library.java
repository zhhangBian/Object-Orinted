...
public class Library {
    ...
    private final ArrayList<BookInfo> borrowInfoMap;
    private final ArrayList<BookInfo> appointmentOffice;
    private final ArrayList<BookInfo> appointmentBuffer;
    private final HashSet<LibraryBookId> driftCorner;
    private final HashMap<LibraryBookId, Integer> driftCountMap;
    private final HashMap<LibraryBookId, Student> driftDonaterMap;
    ...
    public Library(Map<LibraryBookId, Integer> map) {
        ...
        this.borrowInfoMap = new ArrayList<>();
        ...
        this.driftDonaterMap = new HashMap<>();
    ...
    public void QueryCredit(LibraryQcsCmd request) {
        PRINTER.info(request, this.GetStudent(request.getStudentId()).GetCredit());
    }
    ...
    private void Borrow(LibraryReqCmd request, LocalDate date) {
        ...
        if ((bookId.isFormal() && this.bookShelf.get(bookId) <= 0) ||
            ...
            if (student.HaveTypeB() || student.NotInGoodCredit()) {
                this.borrowAndReturnOffice.add(bookId);
                PRINTER.reject(request);
            } else {
                LocalDate dueDate = student.Borrow(bookId, date);
                PRINTER.accept(request);
                this.borrowInfoMap.add(new BookInfo(bookId, studentId, dueDate));
            }
        } else if (bookId.isTypeC()) {
            // 书架到借阅处
            this.bookShelf.put(bookId, this.bookShelf.get(bookId) - 1);

            if (student.HaveBookC(bookId) || student.NotInGoodCredit()) {
                this.borrowAndReturnOffice.add(bookId);
                PRINTER.reject(request);
            } else {
                LocalDate dueDate = student.Borrow(bookId, date);
            ...
            if (student.HaveTypeBU() || student.NotInGoodCredit()) {
                this.borrowAndReturnOffice.add(bookId);
                PRINTER.reject(request);
            } else {
                LocalDate dueDate = student.Borrow(bookId, date);
                PRINTER.accept(request);
                this.borrowInfoMap.add(new BookInfo(bookId, request.getStudentId(), dueDate));
            }
        } else if (bookId.isTypeCU()) {
            // 漂流处到借阅处
            this.driftCorner.remove(bookId);

            if (student.HaveBookCU(bookId) || student.NotInGoodCredit()) {
                this.borrowAndReturnOffice.add(bookId);
                PRINTER.reject(request);
            } else {
                LocalDate dueDate = student.Borrow(bookId, date);
    ...
    private void Order(LibraryReqCmd request, LocalDate date) {
        ...
        if (student.NotInGoodCredit()) {
            PRINTER.reject(request);
        } else if (bookId.isTypeB()) {
            if (student.HaveTypeB() || this.StudentHaveAppointmentBook(bookId, studentId)) {
                PRINTER.reject(request);
            } else {
                PRINTER.accept(request);
                this.appointmentBuffer.add(new BookInfo(bookId, request.getStudentId()));
            }
        } else if (bookId.isTypeC()) {
            if (student.HaveBookC(bookId) || this.StudentHaveAppointmentBook(bookId, studentId)) {
                PRINTER.reject(request);
            } else {
                PRINTER.accept(request);
                this.appointmentBuffer.add(new BookInfo(bookId, request.getStudentId()));
    ...
    private void Return(LibraryReqCmd request, LocalDate date) {
        ...
        this.borrowAndReturnOffice.add(bookId);
        if (notOverDue) {
            this.DeleteBorrowInfo(bookId, studentId);
        }
        student.AddCredit(notOverDue ? 1 : 0);
    ...
    private void Donate(LibraryReqCmd request, LocalDate date) {
        ...
        Student student = this.GetStudent(request.getStudentId());
        this.driftDonaterMap.put(bookId, student);
        student.AddCredit(2);
        PRINTER.accept(request);
    }
    ...
    private void Renew(LibraryReqCmd request, LocalDate date) {
        ...
        if (student.NotInGoodCredit()) {
            PRINTER.reject(request);
        } else if (bookId.isTypeB() || bookId.isTypeC()) {
            ...
            if (success) {
                this.RenewBorrowInfo(bookId, request.getStudentId());
                PRINTER.accept(request);
            } else {
                PRINTER.reject(request);
            }
    ...
    private boolean HaveAppointmentBook(LibraryBookId bookId) {
        // 检查预约buffer
        for (BookInfo bookInfo : this.appointmentBuffer) {
            if (bookInfo.GetBookId().equals(bookId)) {
                return true;
            }
        }
        // 检查预约处
        for (BookInfo bookInfo : this.appointmentOffice) {
            if (bookInfo.GetBookId().equals(bookId)) {
                return true;
            }
        }
    ...
    // 检查预约
    private boolean StudentHaveAppointmentBook(LibraryBookId bookId, String studentId) {
        if (bookId.isTypeB()) {
            // 检查预约buffer
            for (BookInfo bookInfo : this.appointmentBuffer) {
                if (bookInfo.GetBookId().isTypeB() &&
                    bookInfo.GetStudentId().equals(studentId)) {
                    return true;
                }
            }
            // 检查预约处
    ...
    public void AfterClose(LocalDate date) {
        // 检查借书信息，扣信用分
        this.CheckBorrowOverDue(date, true);
        ArrayList<LibraryMoveInfo> moveInfos = new ArrayList<>();
        // 检查预约处是否有过期，扣除响应信用分
        this.CheckAppointmentOffice(date, moveInfos, true);
        PRINTER.move(date, moveInfos);
    }

    public void TidyLibraryWhenOpen(LocalDate date) {
        // 检查借书信息，扣信用分
        this.CheckBorrowOverDue(date, false);
        ArrayList<LibraryMoveInfo> moveInfos = new ArrayList<>();
        // 检查预约处是否有过期，扣除响应信用分
        this.CheckAppointmentOffice(date, moveInfos, false);
        // 检查借还处
        ...
        for (LibraryBookId bookId : this.borrowAndReturnOffice) {
            if (bookId.isFormal()) {
                ...
                if (this.driftCountMap.get(bookId) >= 2) {
                    ...
                    this.driftDonaterMap.get(bookId).AddCredit(2);
                    this.driftDonaterMap.remove(bookId);
                } else {
    ...
    private void CheckAppointmentOffice(LocalDate date, ArrayList<LibraryMoveInfo> moveInfos,
        ...
        Iterator<BookInfo> iterator = this.appointmentOffice.iterator();
        while (iterator.hasNext()) {
            BookInfo bookInfo = iterator.next();
            LibraryBookId bookId = bookInfo.GetBookId();
            LocalDate appointDate = bookInfo.GetDate();

            LocalDate dueDate = appointDate.plusDays(4);
            if ((isAfterClose && (date.isAfter(dueDate) || date.equals(dueDate))) ||
                (!isAfterClose && date.isAfter(dueDate))) {
                this.bookShelf.put(bookId, this.bookShelf.get(bookId) + 1);
                moveInfos.add(new LibraryMoveInfo(bookId, "ao", "bs"));
                iterator.remove();
                // 扣除信用分
                this.GetStudent(bookInfo.GetStudentId()).AddCredit(-3);
            }
        }
    }
    ...
    private void HandleAppointmentRequest(LocalDate date, ArrayList<LibraryMoveInfo> moveInfos) {
        Iterator<BookInfo> iterator = this.appointmentBuffer.iterator();
        while (iterator.hasNext()) {
            BookInfo bookInfo = iterator.next();
            LibraryBookId bookId = bookInfo.GetBookId();
            String studentId = bookInfo.GetStudentId();
            ...
            if (bookNum > 0) {
                ...
                this.appointmentOffice.add(bookInfo);
                iterator.remove();
            }
        }
    }
    ...
    // 检查借书处，扣信用分
    private void CheckBorrowOverDue(LocalDate date, boolean isAfterClose) {
        Iterator<BookInfo> iterator = this.borrowInfoMap.iterator();
        while (iterator.hasNext()) {
            BookInfo bookInfo = iterator.next();
            LocalDate dueDate = bookInfo.GetDate();
            if ((isAfterClose && (dueDate.isBefore(date) || dueDate.equals(date))) ||
                (!isAfterClose && dueDate.isBefore(date))) {
                this.GetStudent(bookInfo.GetStudentId()).AddCredit(-2);
                iterator.remove();
            }
        }
    }
    ...
    private void RenewBorrowInfo(LibraryBookId bookId, String studentId) {
        for (BookInfo bookInfo : this.borrowInfoMap) {
            if (bookInfo.GetBookId().equals(bookId) &&
                bookInfo.GetStudentId().equals(studentId)) {
                bookInfo.RenewDate();
                return;
    ...
    private void DeleteBorrowInfo(LibraryBookId bookId, String studentId) {
        for (int i = 0; i < this.borrowInfoMap.size(); i++) {
            BookInfo bookInfo = this.borrowInfoMap.get(i);
            if (bookInfo.GetBookId().equals(bookId) &&
                bookInfo.GetStudentId().equals(studentId)) {
                this.borrowInfoMap.remove(i);
                return;
            }
        }
    }

    @SendMessage(from = "order", to = "get")
    private void orderNewBook() {

    }

    @SendMessage(from = "get", to = "order")
    private void getOrderedBook() {
