...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        if (validFlag && repeatFlag) {
            Path loop = path.extractLoopPath();
            List<Integer> loopNodes = loop.getNodes();
            assertEquals(oldNodes, path.getNodes());

            for (int i = 0; i < loopNodes.size(); i++) {
                assertTrue(i + posBegin < oldNodes.size());
                assertEquals(loopNodes.get(i), oldNodes.get(i + posBegin));
            }
