...
public class JvmHeap extends MyHeap<MyObject> {//包含题目[1]
    ...
    public void removeUnreferenced() {
        ...
        //[1]
        JvmHeap newHeap = new JvmHeap(this.getSize());
        for (int i = 1; i <= this.getSize(); i++) {
            MyObject myObject = this.getElement(i);
            if (myObject.isReferenced()) {
                newHeap.add(myObject);
            }
        }

        for (int i = 1; i <= newHeap.getSize(); i++) {
            this.setelements(i, newHeap.getElement(i));
        }
        this.setSize(newHeap.getSize());
    }
